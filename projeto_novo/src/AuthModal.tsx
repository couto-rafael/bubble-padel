...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiError(err.message ?? "Erro ao entrar. Verifique email e senha.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const SubmitBtn = ({ label }: { label: string }) => (
    <button
      type="submit"
      disabled={loading}
      className="w-full py-3 bg-gradient-to-r from-[#00ff88] to-[#00dd77] hover:from-[#00dd77] hover:to-[#00cc66] text-[#0a0e27] rounded-lg font-bold transition-all hover:scale-[1.02] shadow-lg disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
    >
                ...
                {[
                  {
                    ...
                    title: "Sou Atleta",
                    desc: "Encontre torneios e acompanhe seu desempenho",
                    icon: (
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                  ...
                  },
                  {
                    ...
                    title: "Sou Clube",
                    desc: "Organize torneios e gerencie competições",
                    icon: (
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
