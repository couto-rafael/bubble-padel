...
function normalizeStatus(s: string) {
  switch (s?.toUpperCase()) {
    ...
    case "PUBLISHED": return "Em Breve";
    default: return s;
  }
}
...
function generateHeatmap(entries: AthleteView["tournaments"]) {
  ...
  const activeDates = new Set(
    entries.map((e) => new Date(e.tournament.startDate + "T12:00:00").toDateString()),
  );
...
const TIER_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  ...
  SILVER: { label: "Prata", color: "text-gray-500", bg: "bg-gray-50 border-gray-200" },
  GOLD: { label: "Ouro", color: "text-yellow-600", bg: "bg-yellow-50 border-yellow-200" },
  DIAMOND: { label: "Diamante", color: "text-cyan-600", bg: "bg-cyan-50 border-cyan-200" },
  LEGEND: { label: "Lenda", color: "text-purple-600", bg: "bg-purple-50 border-purple-200" },
};
...
const TrophyCard = ({ trophy }: { trophy: AthleteTrophy }) => (
  ...
  >
    ...
    <p className="text-[10px] text-gray-300 mt-2">{formatDate(trophy.earnedAt)}</p>
  </div>
);
...
const AchievementCard = ({ achievement }: { achievement: Achievement }) => {
  ...
  const pct =
    achievement.hasProgress && achievement.nextThreshold && achievement.progress > 0
  ...
  return (
    ...
    >
      <span className={`text-2xl flex-shrink-0 mt-0.5 ${!achievement.isUnlocked ? "grayscale" : ""}`}>
        {achievement.icon}
      </span>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          <p className={`font-bold text-[13px] ${achievement.isUnlocked ? "text-gray-900" : "text-gray-400"}`}>
            {achievement.name}
          </p>
          {tierCfg && achievement.isUnlocked && (
            <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded border ${tierCfg.color} ${tierCfg.bg}`}>
              {tierCfg.label}
            </span>
          )}
        </div>
        <p className="text-[12px] text-gray-400 font-normal">{achievement.description}</p>

        {achievement.hasProgress && !achievement.isUnlocked && achievement.nextThreshold && (
          <div className="mt-2">
            <div className="flex items-center justify-between text-[11px] text-gray-400 mb-1">
              <span>{achievement.progress} / {achievement.nextThreshold}</span>
              <span className="font-semibold">{achievement.nextTierLabel}</span>
            </div>
        ...
        {achievement.hasProgress && achievement.isUnlocked && achievement.nextThreshold && (
          <div className="mt-2">
            <div className="flex items-center justify-between text-[11px] text-gray-400 mb-1">
              <span>{achievement.progress} / {achievement.nextThreshold}</span>
              <span className="font-semibold">Próximo: {achievement.nextTierLabel}</span>
            </div>
            <div className="h-1 bg-gray-100 rounded-full overflow-hidden">
              <div
...
const AthleteProfileById: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "tournaments" | "trophies" | "leagues">("overview");
  const [achievementFilter, setAchievementFilter] = useState<"all" | "unlocked" | "inProgress">("all");
  ...
  const TABS = [
    ...
    { id: "tournaments" as const, label: `Torneios${totalTournaments > 0 ? ` (${totalTournaments})` : ""}` },
    { id: "trophies" as const, label: `Troféus${totalTrophies > 0 ? ` (${totalTrophies})` : ""}` },
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fc] pb-24 md:pb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm mb-5">
          ...
          <div className="px-5 pb-5">
            ...
            {((athlete.sports && athlete.sports.length > 0) ||
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {athlete.sports?.map((s) => (
                  ...
                  >
                    {s === "BEACH_TENNIS" ? "🏖️ Beach Tennis" : s === "PADEL" ? "🎾 Padel" : s === "TENIS" ? "🎾 Tênis" : "🏓 Pickleball"}
                  </span>
                ))}
                {athlete.rackets?.map(
                  (r) =>
                    r && (
                      <span key={r} className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 border border-gray-200">
                        🏸 {r}
                      </span>
                    ),
                )}
            ...
            <div className="grid grid-cols-4 gap-2 pt-3 border-t border-gray-100">
              {[
                { value: totalTournaments, label: "Torneios", color: "text-gray-900" },
                { value: totalTrophies, label: "Troféus", color: "text-amber-600" },
                {
                  value: athlete.summary?.winRate != null ? `${athlete.summary.winRate}%` : "—",
                  label: "% Aproveit.",
                  color: "text-blue-600",
                },
                { value: achievUnlocked, label: "Badges", color: "text-purple-600" },
              ].map(({ value, label, color }) => (
                <div key={label} className="text-center">
                  <p className={`text-[20px] font-extrabold tracking-tight leading-none ${color}`}>{value}</p>
          ...
          <div className="space-y-4">
              ...
              {athlete.matchStats ? (
                <>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { label: "Vitórias", value: athlete.matchStats.wins, color: "text-[#00e87a]" },
                      { label: "Derrotas", value: athlete.matchStats.losses, color: "text-red-400" },
                      {
                        label: "Win Rate",
                        value: `${athlete.matchStats.winRate}%`,
                        color: athlete.matchStats.winRate >= 50 ? "text-[#00e87a]" : "text-amber-400",
                      },
                    ].map(({ label, value, color }) => (
                      <div key={label} className="bg-white/[0.05] border border-white/[0.06] rounded-xl p-3 text-center">
                        <p className={`text-[22px] font-black leading-none mb-1 ${color}`}>{value}</p>
                        <p className="text-[10px] font-bold text-[#6b7a99] uppercase tracking-wide">{label}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  {athlete.matchStats.totalMatches > 0 && (
                    <div>
                      <div className="flex items-center justify-between text-[11px] text-[#6b7a99] mb-1.5">
                        <span>{athlete.matchStats.wins}V / {athlete.matchStats.losses}D</span>
                  ...
                  {athlete.matchStats.byCategory.length > 0 && (
                    <div>
                      <p className="text-[11px] font-bold text-[#6b7a99] uppercase tracking-widest mb-2">Por Categoria</p>
                      <div className="space-y-2">
                        {athlete.matchStats.byCategory.map((cat) => (
                          <div key={cat.category} className="flex items-center justify-between text-[12px]">
                            <span className="text-[#9ba5be] font-medium truncate max-w-[120px]">{cat.category}</span>
                            <div className="flex items-center gap-3">
                              <span className="text-[#00e87a] font-bold">{cat.wins}V</span>
                              <span className="text-red-400 font-bold">{cat.losses}D</span>
                              <span className={`font-extrabold text-[11px] ${cat.winRate >= 50 ? "text-[#00e87a]" : "text-amber-400"}`}>
                                {cat.winRate}%
                              </span>
                            </div>
                          </div>
                  ...
                  {athlete.matchStats.topPartners.length > 0 && (
                    <div>
                      <p className="text-[11px] font-bold text-[#6b7a99] uppercase tracking-widest mb-2">Melhores Parceiros</p>
                      <div className="space-y-2">
                        {athlete.matchStats.topPartners.slice(0, 3).map((p) => (
                          <div key={p.name} className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[12px] font-semibold text-[#cdd5e0] truncate max-w-[110px]">{p.name}</span>
                            </div>
                            <div className="flex items-center gap-2 text-[11px]">
                              <span className="text-[#6b7a99]">{p.total} jogos</span>
                              <span className={`font-extrabold ${p.winRate >= 50 ? "text-[#00e87a]" : "text-amber-400"}`}>
                                {p.winRate}%
                              </span>
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {["Vitórias", "Derrotas", "% Aproveit."].map((label) => (
                    <div key={label} className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-3 text-center">
                      ...
                      <p className="text-[10px] font-semibold text-[#6b7a99] uppercase tracking-wide">{label}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <h2 className="text-[14px] font-extrabold text-gray-900 tracking-tight">Últimos Torneios</h2>
              ...
              {athlete.tournaments.length === 0 ? (
                <div className="flex flex-col items-center text-center px-6 py-10">
                  ...
                  <p className="text-[13px] font-bold text-gray-700 mb-1">Ainda sem torneios</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-50">
                  {athlete.tournaments.slice(0, 5).map((entry) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-gray-900 text-[13px] truncate">{entry.tournament.name}</p>
                        <p className="text-[11px] text-gray-500 mt-0.5 font-normal">
                          {entry.tournament.club.name} · {entry.category} · {normalizeSport(entry.tournament.sport)}
                        </p>
                      </div>
                      <span className={`ml-3 text-[11px] font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${
            ...
            {(athlete.achievements?.unlocked ?? []).length > 0 && (
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-[14px] font-extrabold text-gray-900 tracking-tight">Conquistas Recentes</h2>
                  <button
                ...
                <div className="flex flex-wrap gap-2">
                  {(athlete.achievements.unlocked as Achievement[]).slice(0, 4).map((a) => (
                    <div
                      key={a.key}
                      className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${
                        a.currentTier ? TIER_CONFIG[a.currentTier]?.bg : "bg-gray-50 border-gray-200"
                      }`}
                    >
                      <span className="text-lg">{a.icon}</span>
                      <div>
                        <p className="font-bold text-gray-900 text-[12px]">{a.name}</p>
                        {a.currentTier && (
                          <p className={`text-[10px] font-extrabold ${TIER_CONFIG[a.currentTier]?.color}`}>
            ...
            <div className="text-center py-6">
              <p className="text-[12px] text-gray-400 font-normal">📅 Membro desde {memberSince}</p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            {athlete.tournaments.length === 0 ? (
              <div className="flex flex-col items-center py-12 text-center px-6">
                ...
                <p className="text-[13px] font-bold text-gray-700 mb-1">Nenhum torneio ainda</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-50">
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-bold text-gray-900 text-[14px]">{entry.tournament.name}</p>
                        ...
                        <p className="text-[11px] text-gray-400 mt-1 font-normal">
                          Parceiro:{" "}
                          {entry.player1Name.toLowerCase().includes(athlete.fullName.split(" ")[0].toLowerCase())
                            ? entry.player2Name
                            : entry.player1Name}
                        </p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        ...
                        <p className="text-[11px] text-gray-400 mt-2">{formatDate(entry.registrationDate)}</p>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          <div className="space-y-4">
            <div className="bg-[#0a0e1a] border border-white/[0.08] rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-2">
                ...
                <h2 className="text-[14px] font-extrabold text-[#f0f4ff] tracking-tight">Sala de Troféus</h2>
                {totalTrophies > 0 && (
              ...
              <div className="p-5">
                {trophies.length === 0 ? (
                  <div className="flex flex-col items-center text-center py-10">
                    ...
                    <p className="text-[14px] font-bold text-[#f0f4ff] mb-1">Ainda sem troféus</p>
                    <p className="text-[12px] text-[#6b7a99] font-normal">
                      Chegue à final de um torneio para conquistar o primeiro troféu
                    </p>
                  </div>
                ) : (
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                ...
                <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
                  {[
                    ...
                    { key: "unlocked" as const, label: `Desbloqueadas (${athlete.achievements?.unlocked.length ?? 0})` },
                    { key: "inProgress" as const, label: `Em progresso (${athlete.achievements?.inProgress.length ?? 0})` },
                  ].map((f) => (
                    <button
                      key={f.key}
              ...
              {filteredAchievements.length === 0 ? (
                <div className="text-center py-8 text-gray-400 text-sm">Nenhuma conquista neste filtro.</div>
              ) : (
          ...
          <div className="space-y-4">
            {leagueStandings.length === 0 ? (
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-12 flex flex-col items-center text-center">
                ...
                <p className="text-[15px] font-extrabold text-gray-800 mb-1.5">Nenhuma liga ainda</p>
              ...
              leagueStandings.map((standing) => {
                const byCategory = standing.entries.reduce(
                  (acc, e) => {
                    if (!acc[e.category]) acc[e.category] = { points: 0, entries: [] };
                    acc[e.category].points += e.points;
                    acc[e.category].entries.push(e);
                    return acc;
                  },
                  {} as Record<string, { points: number; entries: typeof standing.entries }>,
                );
                const categories = Object.keys(byCategory).sort();

                return (
                  <div key={standing.league.id} className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                    <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
                      <span className="text-lg">🏆</span>
                      <h3 className="font-extrabold text-gray-900 text-[15px] tracking-tight">{standing.league.name}</h3>
                        ...
                        return (
                          <div key={cat} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                            <div className="flex items-center justify-between mb-3">
                              <p className="text-[11px] font-extrabold text-gray-500 uppercase tracking-widest">{cat}</p>
                              <div>
                                <span className="text-[22px] font-extrabold text-gray-900 tracking-tight">{catData.points}</span>
                                ...
                                <div key={i} className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <span className="text-base">
                                      {entry.placement === "champion" ? "🥇" : entry.placement === "runner_up" ? "🥈" : entry.placement === "semi" ? "🥉" : "▸"}
                                    </span>
                                    <span className="text-[12px] text-gray-500 capitalize font-normal">
                                      {entry.placement.replace("_", " ")}
                                    </span>
                                  </div>
                                  <span className="text-[13px] font-bold text-gray-700">+{entry.points} pts</span>
                                </div>
                              ))}
                            </div>
                          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-[#0a0e1a]/95 backdrop-blur-xl border-t border-white/[0.08] pb-safe z-50">
        <div className="flex">
          <Link to="/athlete/dashboard" className="flex-1 flex flex-col items-center gap-1 py-2.5 text-[10px] font-bold text-[#6b7a99] transition-colors">
            <span className="text-xl leading-none">🏠</span>Início
          </Link>
          <Link to="/tournaments" className="flex-1 flex flex-col items-center gap-1 py-2.5 text-[10px] font-bold text-[#6b7a99] transition-colors">
            <span className="text-xl leading-none">🎾</span>Torneios
          </Link>
          <Link to="/athlete/profile" state={{ tab: "trophies" }} className="flex-1 flex flex-col items-center gap-1 py-2.5 text-[10px] font-bold text-[#6b7a99] transition-colors">
            <span className="text-xl leading-none">🏆</span>Troféus
          </Link>
          <Link to="/athlete/profile" className="flex-1 flex flex-col items-center gap-1 py-2.5 text-[10px] font-bold text-[#6b7a99] transition-colors">
