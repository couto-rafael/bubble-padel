...
export default function ClubProfile() {
  ...
  const mapsQuery = encodeURIComponent(
    fullAddress || location || (club.name ?? ""),
  );
    ...
    <div className="min-h-screen bg-[#0a0e1a] text-white">
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="relative -mt-12 mb-6 flex flex-col sm:flex-row sm:items-end gap-4">
          ...
          {/* WhatsApp CTA — direita da identidade, inline sob a capa */}
          {whatsapp && (
            <div className="sm:pb-1 flex-shrink-0">
              <a
                ...
                className="flex items-center gap-2.5 px-5 py-2.5 bg-green-600 hover:bg-green-700 active:bg-green-800 text-white rounded-xl font-bold text-[14px] transition-colors shadow-lg"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
        ...
        <div className="flex flex-wrap items-center gap-3 pb-5 border-b border-white/[0.07]">
          {/* Sport badges */}
          {clubSports.map((s) => (
            <span
              key={s}
        ...
        {/* ── GRID 2 COLUNAS ─────────────────────────────────────────────── */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-6 pb-16">
